...
const data = [
  ...
  { name: "تعليمي", value: 8900, color: "hsl(270, 19%, 55%)" },
  { name: "صحي", value: 6700, color: "hsl(8, 70%, 62%)" },
  { name: "ترفيهي", value: 4200, color: "hsl(25, 72%, 38%)" },
];
  ...
  return (
    <Card className="col-span-full lg:col-span-1">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold">توزيع استخدامات الأراضي</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-80" dir="ltr">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <XAxis type="number" tick={{ fontSize: 12, fill: "hsl(210, 10%, 45%)" }} />
              <YAxis
                dataKey="name"
                type="category"
                tick={{ fontSize: 12, fill: "hsl(210, 18%, 20%)" }}
                width={60}
              />
              <Tooltip
                formatter={(value: number) => [value.toLocaleString("ar-SA"), "المساحة (هكتار)"]}
                contentStyle={{
